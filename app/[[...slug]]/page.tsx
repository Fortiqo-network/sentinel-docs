...
import { allDocParams, fileToHref, loadDoc, loadNavSections, pagerFor } from "@/lib/docs";
...
/**
 * Renders a single markdown doc inside the cinematic shell: sidebar, article,
 * and table of contents. Statically generated from nav.json at build time.
 */
export default async function DocPage({ params }: DocPageProps): Promise<React.JSX.Element> {
  ...
  const href = slug.length ? `/${slug.join("/")}` : fileToHref("index");
