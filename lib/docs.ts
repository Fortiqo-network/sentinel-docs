/**
 * Docs content loader for the standalone Sentinel docs site.
 *
 * Reads the markdown in this repo at build time (every page is statically
 * generated). Navigation order and titles come from `nav.json` + frontmatter.
 */
...
const EMPTY_NAV: DocsNav = { name: "Sentinel", basePath: "/", groups: [] };

/** Read and parse the navigation manifest (empty nav if missing). */
...
function baseRoot(): string {
  const base = loadNav().basePath;
  return base && base !== "/" ? base.replace(/\/$/, "") : "";
}

/** Map a docs file path (nav slug) to its public href. */
export function fileToHref(file: string): string {
  const root = baseRoot();
  if (file === "index") return root || "/";
...
/** Flat, ordered list of every doc link — for prev/next. */
...
export function loadDoc(slug: string[]): LoadedDoc | null {
  ...
  const title =
    typeof parsed.data.title === "string" ? parsed.data.title : humanize(slug.join("/") || "index");
