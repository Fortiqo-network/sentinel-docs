...
<Note>MCP is served through the Sentinel gateway. Exact endpoint paths and aggregate filtering are still evolving — treat the examples as the target shape and check the live API. See [Progress & roadmap](/getting-started/roadmap).</Note>
...
Pass your Sentinel API key in the `X-API-Key` header:
