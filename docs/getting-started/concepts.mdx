...
An **API key** is a long-lived credential prefixed with `sk_live_` (production) or `sk_test_` (sandbox). API keys authenticate REST requests. For user-facing flows, use short-lived [JWTs](#jwt) instead.
