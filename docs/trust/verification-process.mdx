...
<Note>**Live today:** static analysis, supply-chain (CVE), scoring/tiers, and agent **ownership proof**. **Rolling out:** sandboxed dynamic/behavioural and red-team stages (they need the secure runtime) plus SBOM/provenance — until live these are disclosed as *deferred* on a report, never silently passed. See [Progress & roadmap](/getting-started/roadmap).</Note>
